...
logging.basicConfig(filename='SPO_Load1.log', level=logging.INFO,format=formatter)
logging.info('Started\n')

file = "load1/day01.txt"
filename = "Load1_SPO_solutionobjcut_new.csv"
...
for i in range(5):
	...
	clf = SGD_SPO_generic(solver= Gurobi_ICON,accuracy_measure=False,relax=True, 
		validation_relax= True,verbose=True,param= param_data, maximize= False,
	                         epochs= 10,timelimit=21600,obj_cut=1, **h )
	logging.info('SPO')
	start =  time.time()
	pdf = clf.fit(X_1gtrain,y_train,X_1gvalidation,y_validation,X_1gtest,y_test)
	end = time.time()
