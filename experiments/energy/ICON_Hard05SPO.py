...
logging.basicConfig(filename='SPO_hard05.log', level=logging.INFO,format=formatter)
logging.info('Started\n')

file = "../../EnergyCost/Hard_Instances/instance05/instance.txt"
filename = "../Results/Hard05_SPO_timelimit.csv"
modelPATH = str('../Results/Hard05/SPO_hard05')
...
h= {'lr':1e-5}
warmstart_hyperparams = {'baseline': {'reset':True,'presolve':False,'warmstart':False},
...
for k,w in warmstart_hyperparams.items(): 
	for i in range(n_iter): 
		...
		clf = SGD_SPO_generic(solver= Gurobi_ICON,accuracy_measure=False,relax=True, method=1,
		validation_relax= True,param= param_data, maximize= False,n_items=288,model_save=True,model_name= model_name,
	                         epochs= 10,timelimit=21600,lr=1e-5,**w )
		start =  time.time()
