...
def forward_gurobi_prebuilt(Q, p, model, x, inequality_constraints, equality_constraints, G, h, quadobj):
    ...
    try:
        ...
        x_opt = np.array([0 for i in range(len(x))])
        lam = np.array([0 for i in range(len(inequality_constraints))])
        nu = np.array([0 for i in range(len(equality_constraints))])
        print("##############################################")
        print("Something Wrong:Unable to retrieve attribute 'x'")
        print("##############################################")
    ...
    return model.ObjVal, x_opt, nu, lam, slacks,model.Runtime
        ...
        if self.solver == QPSolvers.PDIPM_BATCHED:
            ...
            for i in range(nBatch):
                ...
                if self.model is None:
                    ...
                    vals[i], zhati, nui, lami, si,ti = forward_gurobi_prebuilt(
